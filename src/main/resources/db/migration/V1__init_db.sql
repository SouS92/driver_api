...
create table car (id bigint generated by default as identity, convertible boolean,
date_created timestamp not null, engine_type integer, license_plate varchar(255), rating float, seat_count integer, primary key (id), manufacturer integer);


-- create DRIVER table

create table driver (id bigint generated by default as identity, coordinate binary(255), date_coordinate_updated timestamp,
date_created timestamp not null, deleted boolean not null, online_status varchar(255) not null, password varchar(255) not null,
username varchar(255) not null, car_id bigint, primary key (id));

alter table driver add constraint uc_username unique (username);

alter table driver add constraint constraint_car_id foreign key (car_id) references car;
